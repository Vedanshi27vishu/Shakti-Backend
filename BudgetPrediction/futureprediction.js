...
const predictors = require('./predictor'); // <- sector-wise logic
...
router.post('/predict-budget/:sector', requireAuth, async (req, res) => {
  try {
    ...
    if (!budget) {
      budget = new Budget({ userId, businessType: userBusinessSector, profits: [] });
    }
    ...
    // === Sector-specific prediction logic ===
    ...
    if (lastFourMonths) {
      response.lastFourMonths = lastFourMonths;
    }
