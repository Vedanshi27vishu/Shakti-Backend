...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    const posts = await Post.find({ user: { $in: userIds } })
      .sort({ createdAt: -1 })
      .populate('user', 'Full_Name') // ⬅️ pulls Full_Name from personalDetails
      .lean(); // ⬅️ enables modification of the result object

    // Step 4: Add number of likes & comments to each post
