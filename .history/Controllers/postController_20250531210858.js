...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    // Fetch posts from all matching users sorted by creation date descending
    const posts = await Post.find({ user: { $in: userIds } })
      .sort({ createdAt: -1 });

    // Attach full name to each post by fetching user details
