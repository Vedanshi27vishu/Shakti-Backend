...
const createPost = async (req, res) => {
  try {
    console.log("req.userId:", req.userId); // <--- Check this
...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    // Fetch posts by users in the same business sector
    // Populate user's personal details to get their full name for display
    const posts = await Post.find({ user: { $in: userIds } })
      .sort({ createdAt: -1 })
      .populate({
        path: 'user',
        select: 'personalDetails.Full_Name'  // adapt field name according to your schema
      });
