...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    const username= await PersonalDetails.findOne({userId});
    const name= username.personalDetails.Full_Name;
    // Step 3: Fetch posts by those users
    const posts = await Post.find({ user: { $in: userIds } })
      .sort({ createdAt: -1 })
      .populate('user', 'Full_Name'); // optional: get user's name for display
