...
const PersonalDetails = require('../Models/User/PersonalDetailSignup'); // Add this import
...
const createPost = async (req, res) => {
  try {
    ...
    // Populate the created post with user details
...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    // Step 3: Fetch posts by those users with user details
    ...
    const postsWithCounts = posts.map(post => ({
      ...
      userName: post.user?.personalDetails?.Full_Name || 'Unknown User'
    }));
  ...
  try {
    ...
    res.status(200).json({
      ...
      post: {
        ...
        userName: updatedPost.user?.personalDetails?.Full_Name || 'Unknown User'
      }
    });
  } catch (err) {
...
// Get All Posts with User Details and Counts
