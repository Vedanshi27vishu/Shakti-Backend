...
const Postfetch = require('../Models/PostSchema');

const getPostsByInterest = async (req, res) => {
  try {
    const userId = req.user.userId;

    // Step 1: Get the logged-in user's business sector
    ...
    // Step 2: Find all user IDs who are in the same business sector
    ...
    const posts = await Postfetch.find({
      user: { $in: userIds }
    }).sort({ createdAt: -1 });
