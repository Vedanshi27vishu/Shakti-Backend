...
const getFollowersAndFollowing = async (req, res) => {
  try {
    ...
    const getUserFollowCounts = async (id) => {
      const data = await Follow.findOne({ user: id });
      return {
    ...
    // Map followers with individual stats
    const followerUsers = await Promise.all(
      followData.followers.map(async ({ userId, fullName }) => {
        const { followersCount, followingCount } = await getUserFollowCounts(userId);
        return { userId, fullName, followersCount, followingCount };
      })
    );

    // Map following with individual stats
    const followingUsers = await Promise.all(
      followData.following.map(async ({ userId, fullName }) => {
        const { followersCount, followingCount } = await getUserFollowCounts(userId);
        return { userId, fullName, followersCount, followingCount };
      })
    );
