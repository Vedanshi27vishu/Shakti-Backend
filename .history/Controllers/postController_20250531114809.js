...
const BusinessIdeaDetails = require('../Models/User/PersonalDetailSignup');
...
const getPostsByInterest = async (req, res) => {
  try {
    ...
    // Step 3: Fetch posts by those users with manual lookup for user names
    const posts = await Post.find({ user: { $in: userIds } })
      .sort({ createdAt: -1 });

    console.log("Posts Found:", posts.length);

    // Step 4: Add user names and counts manually
    const postsWithUserInfo = await Promise.all(posts.map(async (post) => {
      let userName = 'Unknown User';
      
      if (post.user) {
        // Find personal details using the user ID from the post
