...
// Update Business City - FIXED TYPO
router.put('/update-business-city', authenticateJWT, async (req, res) => {
  try {
    ...
    // Fixed: Changed from 'Buisness_City' to 'Business_City'
...
// Update Business Location - FIXED FIELD NAME
router.put('/update-business-location', authenticateJWT, async (req, res) => {
  try {
    ...
    // Fixed: Changed from 'Buisness_Location' to 'Business_Location'
    business.ideaDetails.Business_Location = businessLocation;
    business.updatedAt = new Date();
    await business.save();
